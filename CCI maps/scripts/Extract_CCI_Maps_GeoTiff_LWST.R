...
setwd("F:/NetCDFs_CCI/Kariba/2019/2019_LWST_Kariba/")
path <- c("F:/NetCDFs_CCI/Kariba/2019/2019_LWST_Kariba/")
list <- list.files(path, pattern="*.nc$", full.names=TRUE,recursive=TRUE) #- list all files in the directory with extension .nc

path_out <- c("F:/NetCDFs_CCI/out/Kariba_outs/Kariba_LWST_2019_out/")
...
# there MUST BE correspodence to avoid associating wrong quality level to a gven parameter 
...
v1    <- readOGR('F:/NetCDFs_CCI/out/shps/Kariba_shp/Lake_Kariba.shp')  #- path to file on the disk
